...
- [`collateral/one-pager/`](collateral/one-pager/) — the Foresite sales two-pager: page one sells (AI Ads as step one, then Flows, Identities, TV and AI SEO/AEO, proof and pricing), page two explains how it works and links the founder videos. `template.html` is the editable source; `render.mjs` inlines fonts and the logo into `index.html` and prints `Foresite_One_Pager.pdf` with Playwright.
